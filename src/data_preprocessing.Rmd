...
```{r select-data, tidy=TRUE, tidy.opts=list(width.cutoff=40), message=FALSE}
X_selected <- c('cf1edu', 'cm1edu', 'f2b32', 'm2d2', 'cf3marm', 'cf3kids',
...
In this section, we will drop missing values (e.g., not in wave, refuse to respond, skip) in the selected columns in the X variables. 

```{r drop-missing, tidy=TRUE, tidy.opts=list(width.cutoff=40), message=FALSE}

df_missing_handled <- df_selected_x_processed

# Convert all columns to integer
df_missing_handled <- df_missing_handled %>%
  mutate(across(everything(), as.integer))

# Replace negative values in selected columns with NA
df_missing_handled <- df_missing_handled %>%
  mutate(across(all_of(X_selected), ~ifelse(. < 0, NA, .)))

# Write the processed data frame to a Stata file
# write_dta(df_missing_handled, '../data/ff_data_x_preprocessed_v2.dta')
```
